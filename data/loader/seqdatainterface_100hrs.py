...
class SeqDatasetInterface:
    def __init__(self, cfg: BaseConfig, regen_pkl=False):
        ...
        self._seq_path = join(self.data_path, 'Images/')
    ...
    def _get_default_path(self):
        ...
        """
        return f'/data/ford/All_Time_Restricted_Sequences_Blurred'  # f'/home/yash-yee/projects/Ford-GATECH/Models/detection/yolov5/data' # f'/data/ford/active_learning/'
    ...
    def _get_frame_resolution(self, vid):
        path_img_file = join(self._token_to_seq_folder[self._date_time_to_token[vid]], 'processed', 'images')
        # if os.path.isfile(join(self._token_to_seq_folder[self._date_time_to_token[vid]], 'processed/images/00000.pcd.jpg')):
        #     path_img_file = join(self._token_to_seq_folder[self._date_time_to_token[vid]], 'processed/images/00000.pcd.jpg')
        # else:
        #     path_img_file = join(self._token_to_seq_folder[self._date_time_to_token[vid]], 'processed/images/cam1_00000.jpg')
        im_list = os.listdir(path_img_file)
        return plt.imread(os.path.join(path_img_file, im_list[0])).shape[:2]
    ...
    def generate_database(self, filename):
        ...
        for vid in self.video_ids:
            ...
            # except:
            #     print(f'can not get the images or labels for sequence "{vid}"!')
            #     continue
        ...
        for vid in video_ids:
            ...
            for cls, obj_annots in annotations[vid].items():
                ...
                for oid in obj_annots.keys():
                    ...
                    if obj_annots[oid]['frames'][0].startswith('cam1'):
                        images = [join(self._token_to_seq_folder[self._date_time_to_token[vid]], f'processed/images/cam1_{fid:05d}.jpg') for fid in obj_annots[oid]['frames']]
                    else:
                        images = [join(self._token_to_seq_folder[self._date_time_to_token[vid]], f'processed/images/{fid:05d}.pcd.jpg') for fid in obj_annots[oid]['frames']]
    ...
    def get_data_detection(self, image_set, method, filename, **params):
        ...
        for vid in video_ids:
            ...
            for frame in os.listdir(join(join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                                              'processed', 'images'))):
                if not frame.endswith('.json') or not frame.endswith('.mp4'):
                # obj_samples[join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                #                  f'processed/images/{i:05d}.pcd.jpg')] = []
                # if os.path.isfile(join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                #                        f'processed/images/{i:05d}.pcd.jpg')):
                #     obj_samples[join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                #                      f'processed/images/{i:05d}.pcd.jpg')] = []
                    obj_samples[join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                                     f'processed/images/' + frame)] = []
                # else:
                #     obj_samples[join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                #                      f'processed/images/cam1_{i:05d}.jpg')] = []
            for cls, obj_annots in annotations[vid].items():
                if cls in ['num_frames', 'height', 'width']: continue
                ...
                for oid in obj_annots.keys():
                    ...
                    if os.path.isfile(join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                                           f'processed/images/00000.pcd.jpg')):
                        imgs = [join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                                     f'processed/images/{fid:05d}.pcd.jpg') for fid in obj_annots[oid]['frames']]
                    else:
                        imgs = [join(self._token_to_seq_folder[self._date_time_to_token[vid]],
                                     f'processed/images/cam1_{fid:05d}.jpg') for fid in obj_annots[oid]['frames']]
                    boxes = obj_annots[oid]['bbox']
    ...
    def _get_all_annotations_obj(self, vid):
        ...
        if self._date_time_to_token[vid] in self._token_to_label_file.keys():
            path_to_file = self._token_to_label_file[self._date_time_to_token[vid]]  ## vid-->token-->label file
        else:
            print(vid)
            print(self._date_time_to_token[vid])
            print(self._token_to_label_file)
    ...
    def _generate_csv_yolo(self, image_set, file_path, obj_samples):
        ...
        """
        data_save_path = join(file_path, 'yolo_focal_timerestricted_' + image_set + '.txt')
            ...
            with open(data_save_path, "wt") as f:
                for img, samples in sorted(obj_samples.items()):
                    ...
                    if img not in corrupted.keys():
                        ...
                        for s in samples:
                            box = s['box']
                            if box[2] > 50 and box[3] > 50:  # if box[2] > 50 and box[3] > 75:
                                if not written:
                                    written = True
                                    f.write('%s ' % (img))
                                f.write('%.0f,%.0f,%.0f,%.0f,%.0f ' % (box[0], box[1], box[2], box[3], s['label']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', help='Path to input config file', type=str,
                        default='../../structure_focal_config.toml')
    ...
    for split in splits:
        data_interface.get_data_detection(image_set=split, method='yolo', filename='focal_tmp_database.pkl')
